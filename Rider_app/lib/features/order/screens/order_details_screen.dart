  ...
  _loadData() {
    if(orderModel?.orderAmount == null) {
      Provider.of<OrderProvider>(context, listen: false).getOrderModel('${orderModel!.id}').then((OrderModel? value) {
        ...
        Provider.of<OrderProvider>(Get.context!, listen: false).getOrderDetails(orderModel!.id.toString(), Get.context!).then((value) {
          Provider.of<TimerProvider>(Get.context!, listen: false).countDownTimer(orderModel!, Get.context!);
        });
      });
    }else{
      if(orderModel?.orderType == 'delivery') {
        ...
        child: Consumer<OrderProvider>(
          builder: (context, order, child) {
            ...
            if (order.orderDetails != null && orderModel!.orderAmount != null) {
              ...
              totalPrice = (itemsPrice + tax + addOns) - discount + deliveryCharge! - orderModel!.couponDiscountAmount!;
                ...
                Expanded(
                  child: ListView(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          ListTile(
                            ...
                            title: Text( orderModel?.deliveryAddress?.contactPersonName ?? "${orderModel?.customer?.fName ?? ""} ${orderModel?.customer?.lName ?? ""}"  ,
                          ...
                          if(orderModel?.deliveryAddress != null) ListTile(
                            visualDensity: VisualDensity.compact,
                            leading: const Icon(Icons.location_on_rounded, size: 20),
                            horizontalTitleGap: Dimensions.paddingSizeExtraSmall,
                            title: Text(orderModel?.deliveryAddress?.address ?? "", style: rubikRegular.copyWith(fontSize: Dimensions.fontSizeDefault)),
                            onTap: (){
                      ...
                      if( orderModel !=null && orderModel?.bringChangeAmount !=null && orderModel!.bringChangeAmount! > 0)Container(
                        decoration: BoxDecoration(
                        ...
                        child: RichText(text: TextSpan(children: [
                          ...
                          TextSpan(text: " ${PriceConverterHelper.convertPrice(context, orderModel?.bringChangeAmount)} ",
                      ...
                      child: SliderButtonWidget(
                        action: () {
                          LocationHelper.checkPermission(context, callBack: () async {
                            ...
                            order.getCurrentOrdersList(1, context);
                            order.getOrderHistoryList(1, context);
