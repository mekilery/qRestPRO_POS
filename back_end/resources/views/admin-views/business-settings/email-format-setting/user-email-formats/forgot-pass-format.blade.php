    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Model\BusinessSetting::where('key','forget_password_mail_status_user')->first())
                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input" onclick="toogleStatusModal(event,'mail-status','place-order-on.png','place-order-off.png','{{translate('Want_to_enable_the')}} <strong>{{translate('Forgot_Password')}}</strong> {{translate('mail')}} ?','{{translate('Want_to_disable_the')}} <strong>{{translate('Forgot_Password')}}</strong> {{translate('mail')}} ?',`<p>{{translate('If_enabled,_the_Customer_will_receive_an_automated_email_with_a_Reset_Password_link.')}}</p>`,`<p>{{translate('If_disabled,_the_Customer_will_not_receive_any_for_password_reset.')}}</p>`)" id="mail-status" {{$mail_status == '1'?'checked':''}}>
                ...
                <form action="{{ route('admin.business-settings.email-setup.update', ['user','forget-password']) }}" method="POST" enctype="multipart/form-data" id="order-form">
                    @csrf
                                        ...
                                        @if ($language)
                                            <div class="__bg-F8F9FC-card default-form lang_form" id="default-form">
                                                ...
                                                <div class="form-group mb-0">
                                                    ...
                                                    <div id="editor-default" class="min-h-108px" data-id="mail-body">{!! $data?->body ?? '' !!}</div>
                                                    <textarea class="form-control quill-hidden" id="ckeditor-default" name="body[]" data-id="mail-body" style="display:none;"></textarea>
                                                </div>
                                            </div>
                                                ...
                                                <div class="__bg-F8F9FC-card d-none lang_form" id="{{$lang->code}}-form">
                                                    ...
                                                    <div class="form-group mb-0">
                                                       <label class="form-label">
                                                        ...
                                                        <div id="editor-{{$lang->code}}" class="min-h-108px"> {!! $translate[$lang->code]['body']??'' !!}</div>
                                                        <textarea class="form-control" id="ckeditor-{{$lang->code}}" name="body[]" style="display:none;"></textarea>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="__bg-F8F9FC-card">
                                            ...
                                            <div class="form-group mb-0">
                                                @if ($language)
                                                       <div class="form-group lang_form default-form">
                                                            ...
                                                            <input type="text" maxlength="50" data-id="mail-copyright" name="copyright_text[]"  placeholder="{{ translate('Ex:_Copyright_2023_qRestPRO._All_right_reserved')}}" class="form-control" value="{{ $data?->getRawOriginal('copyright_text') }}">
                                                        </div>
                                                    @foreach(json_decode($language) as $lang)
                                                    <?php
                                           $translate = [];
                                           if($data && count($data['translations'])){
                                                        ...
                                                        <div class="form-group d-none lang_form" id="{{$lang->code}}-form3">
                                                            ...
                                                            <input type="text" maxlength="50" name="copyright_text[]"  placeholder="{{ translate('Ex:_Copyright_2023_qRestPRO._All_right_reserved')}}" class="form-control" value="{{ $translate[$lang->code]['copyright_text']??'' }}">
                                                        </div>
                                                    @endforeach
                                                @else
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" maxlength="50"  placeholder="{{ translate('Ex:_Copyright_2023_qRestPRO._All_right_reserved')}}"class="form-control" name="copyright_text[]" value="">
                                                </div>
                                                @endif
                                            </div>
                                        </div>
...
@push('script_2')
    ...
    <script>
        $(document).ready(function () {
            ...
            // Initialize Quill for the default editor
            if (!$('#editor-default').hasClass('ql-container')) {
                var defaultQuill = new Quill('#editor-default', {
                    modules: {
                ...
                // Live Preview Update
                defaultQuill.on('text-change', function () {
                    $('#mail-body').html(defaultQuill.root.innerHTML);  // Live preview
                    $('#ckeditor-default').val(defaultQuill.root.innerHTML); // Store data in hidden field
                });

                // On form submit, store default Quill content in the hidden textarea
                $('#order-form').on('submit', function () {
                    $('#ckeditor-default').val(defaultQuill.root.innerHTML);
                });
            }

            // Loop through each language form and initialize Quill editors
            $('.lang_form').each(function () {
                let langCode = $(this).attr('id').replace('-form', ''); // Extract language code

                if ($(`#editor-${langCode}`).length && !$(`#editor-${langCode}`).hasClass('ql-container')) {
                    let quillEditor = new Quill(`#editor-${langCode}`, {
                        modules: {
                            toolbar: toolbarOptions
                        },
                        theme: 'snow'
                    ...
                    // Store language-specific content in hidden textarea before submission
                    $('#order-form').on('submit', function () {
                        $(`#ckeditor-${langCode}`).val(quillEditor.root.innerHTML);
                    });
                }
            });
        });
